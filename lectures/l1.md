---
layout: module
title: "1 - Orientation & User Stories"
permalink: /modules/1-user-stories
parent: Modules
nav_order: 1
lessons: 
  - title: Course Introduction
    pdf: "Module 01.1 Course Introduction.pdf"
    ppt: "Module 01.1 Course Introduction.pptx"
  - title: User Stories
    pdf: "Module 01.2 User Stories.pdf" 
    ppt: "Module 01.2 User Stories.pptx"
---
### Learning Objectives:
After this lecture, you will be able to:

* Explain in general terms what software engineering is 
* List your weekly obligations as a student
* List the requirements for completing the course
* Explain the structure of a user story
* Identify and fix user stories that don’t have the correct structure
* Define the relationship between conditions of satisfaction and user stories, and the difference between essential, desired, and extension conditions of satisfaction
* Identify functional and non-functional requirements, and give examples of each  

### Important Dates:
* Individual Project 1 **Due Wednesday, May 13 by 6pm (EST)**{: .label .label-red }

{% include lesson.html %}

### Activity
* [Working with User Stories]({{site.baseurl}}{% link Activities/Module01 Activity.md %})

### Tutorials
* [Installing a Development Environment]({{site.baseurl}}{% link tutorials/week1-getting-started.md %}) 
* [Getting Started with TypeScript]({{site.baseurl}}{% link tutorials/week1-typescript.md %})
* [Guide to API Requests and Installing Postman]({{site.baseurl}}{% link tutorials/week1-api-requests-postman.md %})


### Resources
* Class [Syllabus]({{site.baseurl}}{% link index.md %})  and [Policies]({{site.baseurl}}{% link policies.md %}) 
* [INVEST criteria for user stories](https://agileforall.com/new-to-agile-invest-in-good-user-stories/)
* [New to agile? INVEST in good user stories](https://agileforall.com/new-to-agile-invest-in-good-user-stories/) — introduction to both user stories and INVEST criteria
* [Domain modeling made functional](https://onesearch.library.northeastern.edu/permalink/01NEU_INST/i2gqis/alma9952083043301401) - the first section in Chapter 2, "Interview with a Domain Expert," has a worked example of soliciting a client's needs for software.
* [Project Overview]({{site.baseurl}}{% link Assignments/project-overview.md %})
